...
# GPU acceleration imports (with fallback handling)
try:
    import cupy as cp
    import cupyx.scipy.spatial.distance as gpu_distance
    GPU_AVAILABLE = True
    print("[PARALLEL] CuPy detected - GPU acceleration available")
except ImportError:
    cp = None
    gpu_distance = None
    GPU_AVAILABLE = False
    print("[PARALLEL] CuPy not available - using CPU/Serial fallback")

try:
    from numba import cuda, jit, prange
    NUMBA_AVAILABLE = True
    print("[PARALLEL] Numba detected - JIT compilation available")
except ImportError:
    cuda = None
    jit = None
    prange = range
    NUMBA_AVAILABLE = False
    print("[PARALLEL] Numba not available - using standard Python")
...
OUTPUT_FILENAME = "Baseline-45-cars-500seconds-omnidirectional-L3.xlsx"  # Set to None for automatic naming, or specify custom name
...
BATCH_SIZE_PER_GPU_GB = 1000       # Vehicles per GB of GPU memory
DISTANCE_MATRIX_CHUNK_SIZE = 2000  # Chunk size for distance matrix calculation




class VehicleDataBuffer:
    """Thread-safe buffer for vehicle data during parallel processing"""
    
    def __init__(self):
        self.lock = threading.RLock()
        self.input_data = {}      # Raw input for current timestamp
        self.temp_calculations = {} # Intermediate calculations
        self.final_results = {}   # Final output ready for application
        self.phase_complete = {}  # Track completion status
        
    def set_input_data(self, vehicle_id: str, data: dict):
        with self.lock:
            self.input_data[vehicle_id] = copy.deepcopy(data)
    
    def get_input_data(self, vehicle_id: str) -> dict:
        with self.lock:
            return copy.deepcopy(self.input_data.get(vehicle_id, {}))
    
    def set_temp_calculation(self, vehicle_id: str, calc_type: str, result):
        with self.lock:
            if vehicle_id not in self.temp_calculations:
                self.temp_calculations[vehicle_id] = {}
            self.temp_calculations[vehicle_id][calc_type] = copy.deepcopy(result)
    
    def get_temp_calculation(self, vehicle_id: str, calc_type: str):
        with self.lock:
            return copy.deepcopy(
                self.temp_calculations.get(vehicle_id, {}).get(calc_type)
            )
    
    def set_final_result(self, vehicle_id: str, results: dict):
        with self.lock:
            self.final_results[vehicle_id] = copy.deepcopy(results)
    
    def get_final_result(self, vehicle_id: str) -> dict:
        with self.lock:
            return copy.deepcopy(self.final_results.get(vehicle_id, {}))
    
    def clear_all(self):
        with self.lock:
            self.input_data.clear()
            self.temp_calculations.clear()
            self.final_results.clear()
            self.phase_complete.clear()
    
    def get_all_vehicle_ids(self) -> set:
        with self.lock:
            return set(self.input_data.keys())

class ParallelProcessingBarrier:
    """Barrier synchronization for phase-based parallel processing"""
    ...
    def wait_all_complete(self, phase_name: str = ""):
        """Wait for all workers to complete current phase"""
        try:
            self.barrier.wait(timeout=30.0)  # 30 second timeout
            if self.error_occurred.is_set():
                raise RuntimeError(f"Error in parallel phase {phase_name}: {self.error_info}")
        except threading.BrokenBarrierError as e:
            raise RuntimeError(f"Barrier broken in phase {phase_name}: {e}")
    
    def signal_error(self, error_info: str):
        """Signal that an error occurred in one of the workers"""
        with self.lock:
    ...
    def reset(self):
        """Reset barrier for next timestamp"""
        with self.lock:
...
class ParallelComputationManager:
    ...
    def __init__(self, config):
        ...
        # NEW: Add buffer and barrier support
        self.vehicle_buffer = VehicleDataBuffer()
        self.processing_barrier = None
        self.num_workers = 0
        
        # Simple GPU selection (existing code...)
        ...
        # FIXED: Safe batch size initialization (existing code...)
        try:
        ...
        self.cpu_cores = max(1, int(mp.cpu_count() * CPU_CORE_RATIO))
        self.gpu_available = GPU_AVAILABLE
        self.performance_history = {'gpu': [], 'cpu': [], 'serial': []}
        
        # Use thread-based parallelism for CPU to avoid pickling issues
        self.use_thread_parallelism = True
        
        self._initialize_computation_mode()
        
        print(f"[PARALLEL] Initialized with mode: {self.computation_mode}")
        print(f"[PARALLEL] Batch size: {self.optimal_batch_size}")
        print(f"[PARALLEL] CPU cores: {self.cpu_cores}")
        print(f"[PARALLEL] Buffer-based synchronization enabled")
    ...
    def _test_gpu_computation(self):
        ...
        try:
            test_data = cp.random.random((100, 2))
            distances = cp.linalg.norm(test_data[:, None] - test_data[None, :], axis=2)
            cp.cuda.Stream.null.synchronize()
            print("[PARALLEL] GPU test passed")
            return True
        except Exception as e:
            print(f"[PARALLEL] GPU test failed: {e}")
            return False
    
    def _test_cpu_computation(self):
            """Test CPU computation using thread-based parallelism"""
            try:
                ...
                with ThreadPoolExecutor(max_workers=self.cpu_cores) as executor:
                    ...
                    if results != expected:
                        print(f"[PARALLEL] Thread test failed: {results} != {expected}")
                        return False
                
                # Test 2: More realistic workload
                def compute_distances(data_chunk):
                    """Compute distances for a chunk of data"""
                    n = len(data_chunk)
                    distances = []
                    for i in range(n):
                        for j in range(i+1, n):
                            dist = ((data_chunk[i][0] - data_chunk[j][0])**2 + 
                                   (data_chunk[i][1] - data_chunk[j][1])**2)**0.5
                            distances.append(dist)
                    return distances
                
                # Create test data
                test_data = [(i*10, i*5) for i in range(20)]
                chunk_size = 5
                chunks = [test_data[i:i+chunk_size] for i in range(0, len(test_data), chunk_size)]
                
                with ThreadPoolExecutor(max_workers=2) as executor:
                    futures = [executor.submit(compute_distances, chunk) for chunk in chunks]
                    chunk_results = [f.result() for f in futures]
                    
                    # Verify we got results
                    if all(isinstance(result, list) and len(result) >= 0 for result in chunk_results):
                        print("[PARALLEL] Thread-based CPU test passed")
                        return True
                    else:
                        print("[PARALLEL] Thread-based CPU test failed: invalid results")
                        return False
                        
            except Exception as e:
                print(f"[PARALLEL] Thread-based CPU test failed: {e}")
                return False
    
    def get_cpu_executor(self, max_workers=None):
        """Get appropriate CPU executor (thread-based to avoid pickling)"""
    ...
    def adaptive_fallback(self, error_context: str):
        """Adaptive fallback when current mode fails"""
        print(f"[PARALLEL] {error_context} failed in {self.computation_mode} mode")
        
        if self.computation_mode == 'gpu':
            print("[PARALLEL] Falling back to CPU parallel processing")
            self.computation_mode = 'cpu'
        elif self.computation_mode == 'cpu':
            print("[PARALLEL] Falling back to serial processing")
            self.computation_mode = 'serial'
        else:
            print("[PARALLEL] Already in serial mode - cannot fallback further")
    
    def get_computation_mode(self):
        """Get current computation mode"""
        return self.computation_mode
    
    def get_performance_metrics(self):
        """Get real-time performance metrics for the parallel computation manager"""
        metrics = {
            'current_mode': self.computation_mode,
        ...
        if GPU_AVAILABLE and self.computation_mode == 'gpu':
            try:
                mempool = cp.get_default_memory_pool()
                metrics['gpu_memory_used_gb'] = mempool.used_bytes() / (1024**3)
                metrics['gpu_memory_total_gb'] = mempool.total_bytes() / (1024**3)
                metrics['gpu_memory_utilization'] = (mempool.used_bytes() / max(mempool.total_bytes(), 1)) * 100
            except:
        ...
        if self.computation_mode in ['cpu', 'serial']:
            try:
                import psutil
        ...
        return metrics
    def setup_barrier_processing(self, vehicle_count: int):
        """Setup barrier processing for current timestamp"""
        if vehicle_count < MIN_VEHICLES_FOR_PARALLEL:
            self.num_workers = 1
            self.processing_barrier = None
            return 'serial'
        ...
        if self.computation_mode == 'gpu':
            self.num_workers = min(4, self.cpu_cores)  # GPU + helper threads
        elif self.computation_mode == 'cpu':
            self.num_workers = min(self.cpu_cores, max(2, vehicle_count // 10))
        else:
            self.num_workers = 1
        
        if self.num_workers > 1:
            self.processing_barrier = ParallelProcessingBarrier(self.num_workers)
            print(f"[PARALLEL] Setup barrier with {self.num_workers} workers")
        else:
            self.processing_barrier = None
        
        self.vehicle_buffer.clear_all()
        return self.computation_mode

class VehicleDataBatch:
    """Efficient batch container for vehicle data"""
    
    def __init__(self, vehicle_ids, positions, headings, powers, mcs_values, beacon_rates):
        self.vehicle_ids = vehicle_ids
        self.positions = positions  # Nx2 array
        self.headings = headings    # N array
        self.powers = powers        # N array
        self.mcs_values = mcs_values  # N array
        self.beacon_rates = beacon_rates  # N array
        self.size = len(vehicle_ids)
    
    def to_gpu(self):
        """Convert to GPU arrays if available"""
        if GPU_AVAILABLE:
            return VehicleDataBatch(
                self.vehicle_ids,  # Keep IDs on CPU
                cp.asarray(self.positions),
                cp.asarray(self.headings),
                cp.asarray(self.powers),
                cp.asarray(self.mcs_values),
                cp.asarray(self.beacon_rates)
            )
        return self
    
    def to_cpu(self):
        """Convert to CPU arrays"""
        if GPU_AVAILABLE and hasattr(self.positions, 'get'):
            return VehicleDataBatch(
                self.vehicle_ids,
                self.positions.get(),
                self.headings.get(),
                self.powers.get(),
                self.mcs_values.get(),
                self.beacon_rates.get()
            )
        return self


class ParallelDistanceCalculator:
    """Vectorized distance calculation with GPU acceleration"""
    
    def __init__(self, parallel_manager):
        self.parallel_manager = parallel_manager
    
    def calculate_distance_matrix(self, positions):
        """Calculate pairwise distance matrix efficiently"""
        mode = self.parallel_manager.get_computation_mode()
        
        try:
            if mode == 'gpu':
                return self._gpu_distance_matrix(positions)
            elif mode == 'cpu':
                return self._cpu_distance_matrix(positions)
            else:
                return self._serial_distance_matrix(positions)
        except Exception as e:
            self.parallel_manager.adaptive_fallback(f"Distance matrix calculation")
            # Retry with fallback mode
            return self.calculate_distance_matrix(positions)
    
    def _gpu_distance_matrix(self, positions):
        ...
        return distances.get()  # Convert back to CPU for compatibility
    
    def _cpu_distance_matrix(self, positions):
        """CPU parallel distance matrix calculation"""
    ...
    def _serial_distance_matrix(self, positions):
        """Serial distance matrix calculation"""
        n = len(positions)
...
class ParallelNeighborFinder:
    """Parallel neighbor finding with batched processing"""
    
    def __init__(self, parallel_manager, interference_calculator):
        self.parallel_manager = parallel_manager
        self.interference_calculator = interference_calculator
        self.distance_calculator = ParallelDistanceCalculator(parallel_manager)
    
    def find_all_neighbors_parallel(self, vehicle_batch, current_time: float, vehicles_dict) -> dict:
        """Find neighbors for all vehicles in parallel"""
        mode = self.parallel_manager.get_computation_mode()
        
        try:
            if vehicle_batch.size < MIN_VEHICLES_FOR_PARALLEL:
                return self._serial_neighbor_finding(vehicle_batch, current_time, vehicles_dict)
            
            if mode == 'gpu':
                return self._gpu_neighbor_finding(vehicle_batch, current_time, vehicles_dict)
            elif mode == 'cpu':
                return self._cpu_neighbor_finding(vehicle_batch, current_time, vehicles_dict)
            else:
                return self._serial_neighbor_finding(vehicle_batch, current_time, vehicles_dict)
        except Exception as e:
            self.parallel_manager.adaptive_fallback("Neighbor finding")
            return self._serial_neighbor_finding(vehicle_batch, current_time, vehicles_dict)
    
    def _serial_neighbor_finding(self, vehicle_batch, current_time: float, vehicles_dict) -> dict:
        """Serial neighbor finding using existing logic"""
        all_neighbors = {}
        
        for i, vehicle_id in enumerate(vehicle_batch.vehicle_ids):
            neighbors = []
            vehicle_range = self._calculate_single_comm_range(vehicle_batch, i)
            
            for j, neighbor_id in enumerate(vehicle_batch.vehicle_ids):
                if i != j:
                    distance = np.sqrt(np.sum((vehicle_batch.positions[i] - vehicle_batch.positions[j])**2))
                    
                    if distance <= vehicle_range:
                        neighbor_info = self._create_neighbor_info(
                            vehicle_id, neighbor_id, distance,
                            vehicle_batch, i, j, current_time, vehicles_dict
                        )
                        if neighbor_info:
                            neighbors.append(neighbor_info)
            
            neighbors.sort(key=lambda x: x['distance'])
            all_neighbors[vehicle_id] = neighbors
        
        return all_neighbors
    
    def _gpu_neighbor_finding(self, vehicle_batch, current_time: float, vehicles_dict) -> dict:
        """GPU-accelerated neighbor finding"""
        if not GPU_AVAILABLE:
            raise RuntimeError("GPU not available")
        
        # Calculate distance matrix on GPU
        distances = self.distance_calculator._gpu_distance_matrix(vehicle_batch.positions)
        
        # Process neighbors using GPU-computed distances
        return self._process_neighbors_with_distances(vehicle_batch, distances, current_time, vehicles_dict)
    
    def _cpu_neighbor_finding(self, vehicle_batch, current_time: float, vehicles_dict) -> dict:
            """CPU parallel neighbor finding using thread-based approach"""
            from concurrent.futures import ThreadPoolExecutor
            
            all_neighbors = {}
            
            def process_vehicle_chunk(vehicle_indices):
                """Process a chunk of vehicles for neighbor finding"""
                chunk_neighbors = {}
                for i in vehicle_indices:
                    if i >= len(vehicle_batch.vehicle_ids):
                        continue
                        
                    vehicle_id = vehicle_batch.vehicle_ids[i]
                    neighbors = []
                    vehicle_range = self._calculate_single_comm_range(vehicle_batch, i)
                    ...
                    neighbors.sort(key=lambda x: x['distance'])
                    chunk_neighbors[vehicle_id] = neighbors
                
                return chunk_neighbors
            
            # Split vehicles into chunks for parallel processing
            vehicle_count = len(vehicle_batch.vehicle_ids)
            chunk_size = max(1, vehicle_count // self.parallel_manager.cpu_cores)
            chunks = [list(range(i, min(i + chunk_size, vehicle_count))) 
                     for i in range(0, vehicle_count, chunk_size)]
            
            try:
                with ThreadPoolExecutor(max_workers=self.parallel_manager.cpu_cores) as executor:
                    futures = [executor.submit(process_vehicle_chunk, chunk) for chunk in chunks]
                    
                    for future in futures:
                        chunk_result = future.result()
                        all_neighbors.update(chunk_result)
                        
            except Exception as e:
                print(f"[PARALLEL] Thread-based CPU neighbor finding failed: {e}")
                # Fallback to serial processing
                return self._serial_neighbor_finding(vehicle_batch, current_time, vehicles_dict)
            
            return all_neighbors
    
    def _process_neighbors_with_distances(self, vehicle_batch, distances, current_time: float, vehicles_dict) -> dict:
        """Process neighbors using pre-computed distance matrix"""
        all_neighbors = {}
        ...
        for i, vehicle_id in enumerate(vehicle_batch.vehicle_ids):
            ...
            for j, neighbor_id in enumerate(vehicle_batch.vehicle_ids):
                if i != j and distances[i, j] <= vehicle_range:
                    neighbor_info = self._create_neighbor_info(
                        vehicle_id, neighbor_id, distances[i, j],
                        vehicle_batch, i, j, current_time, vehicles_dict
                    )
                    if neighbor_info:
                        neighbors.append(neighbor_info)
    ...
    def _calculate_single_comm_range(self, vehicle_batch, index: int) -> float:
        """Calculate communication range for a single vehicle"""
        power = vehicle_batch.powers[index]
        return self.interference_calculator.calculate_dynamic_communication_range(
            power, self.interference_calculator.config.channel_model
        )
    
    def _create_neighbor_info(self, vehicle_id: str, neighbor_id: str, distance: float,
                             vehicle_batch, v_idx: int, n_idx: int,
                             current_time: float, vehicles_dict) -> dict:
        """Create neighbor info structure"""
        vehicle_heading = vehicle_batch.headings[v_idx]
        neighbor_pos = vehicle_batch.positions[n_idx]
        vehicle_pos = vehicle_batch.positions[v_idx]
        
        # Calculate angle to neighbor
        angle_to_neighbor = math.degrees(math.atan2(
            neighbor_pos[1] - vehicle_pos[1],
            neighbor_pos[0] - vehicle_pos[0]
        )) % 360
        
        # Convert from atan2 to FCD coordinate system
        angle_to_neighbor_fcd = (90 - angle_to_neighbor) % 360
        ...
        return {
            'id': neighbor_id,
            'distance': distance,
            'angle_to_neighbor': angle_to_neighbor_fcd,
            'relative_angle': relative_angle,
            'vehicle_heading': vehicle_heading,
            'neighbor_sector': sector,
            'tx_power': vehicle_batch.powers[n_idx],
            'antenna_sector': sector,
            'beacon_rate': vehicle_batch.beacon_rates[n_idx],
            'mcs': vehicle_batch.mcs_values[n_idx],
...
class BackgroundTrafficManager:
    ...
    def get_effective_cbr_contribution(self, vehicle_id: str, base_cbr: float) -> float:
        """ENHANCED CBR contribution with stronger background traffic impact"""
        ...
        # ENHANCED: More realistic scaling with IEEE 802.11bd channel capacity
        channel_capacity_mbps = 25.0  # More conservative estimate for realistic conditions
        
        # ENHANCED: More aggressive CBR contribution from background traffic
        mgmt_cbr_contribution = (vehicle_load["management_mbps"] / channel_capacity_mbps) * 1.2    # Increased
        info_cbr_contribution = (vehicle_load["infotainment_mbps"] / channel_capacity_mbps) * 1.5  # Increased
        
        # ENHANCED: More aggressive additional CBR from packet overhead and contention
        total_bg_load = vehicle_load["total_mbps"]
        if total_bg_load > 0:
            contention_overhead = (total_bg_load / channel_capacity_mbps) * 0.7    # Increased from 0.5
            collision_overhead = (total_bg_load / channel_capacity_mbps) * 0.4     # Increased from 0.3
        ...
        # ENHANCED: Full background traffic load impact
        background_cbr_contribution *= BACKGROUND_TRAFFIC_TOTAL_LOAD  # Remove 0.8 reduction factor
        
        # ENHANCED: More aggressive scaling for high background loads
        if background_cbr_contribution > 0.06:  # Lower threshold
            background_cbr_contribution *= 1.3  # Increased amplification
...
class VANET_IEEE80211bd_L3_SDN_Simulator:
    ...
    def _process_vehicles_with_barriers(self, current_vehicles, current_time):
        """Process vehicles using barrier synchronization"""
        
        # PHASE 1: Collect input data (parallel preparation)
        print(f"[BARRIER] Phase 1: Collecting input data for {len(current_vehicles)} vehicles")
        
        def collect_vehicle_data(vehicle_id):
            """Collect input data for one vehicle"""
            try:
                ...
                if vehicle_data:
                    input_data = {
                        ...
                        'transmission_power': self.vehicles[vehicle_id].transmission_power,
                        'mcs': self.vehicles[vehicle_id].mcs,
                        'beacon_rate': self.vehicles[vehicle_id].beacon_rate,
                        'vehicle_obj': copy.deepcopy(self.vehicles[vehicle_id])
                    }
                    
                    self.parallel_manager.vehicle_buffer.set_input_data(vehicle_id, input_data)
                    return True
                return False
            except Exception as e:
                print(f"[BARRIER ERROR] Data collection failed for {vehicle_id}: {e}")
                return False
        
        # Execute phase 1 in parallel
        success_count = 0
        if self.parallel_manager.num_workers > 1:
            with ThreadPoolExecutor(max_workers=self.parallel_manager.num_workers) as executor:
                futures = [executor.submit(collect_vehicle_data, vid) for vid in current_vehicles]
                for future in as_completed(futures, timeout=10):
                    if future.result():
                        success_count += 1
        else:
            for vehicle_id in current_vehicles:
                if collect_vehicle_data(vehicle_id):
                    success_count += 1
        
        if success_count == 0:
            return False
        
        # BARRIER 1: Wait for all data collection to complete
        if self.parallel_manager.processing_barrier:
            try:
                self.parallel_manager.processing_barrier.wait_all_complete("Data Collection")
            except Exception as e:
                print(f"[BARRIER ERROR] Phase 1 barrier failed: {e}")
                return False
        
        # PHASE 2: Find neighbors using consistent data (parallel)
        print(f"[BARRIER] Phase 2: Finding neighbors with consistent data")
        
        def find_neighbors_for_vehicle(vehicle_id):
            """Find neighbors using buffer data"""
            try:
                input_data = self.parallel_manager.vehicle_buffer.get_input_data(vehicle_id)
                if not input_data:
                    return False
                
                # Find neighbors using CONSISTENT snapshot data
                neighbors = self._find_neighbors_with_buffer_data(vehicle_id, current_time, input_data)
                
                self.parallel_manager.vehicle_buffer.set_temp_calculation(
                    vehicle_id, 'neighbors', neighbors
                )
                return True
            except Exception as e:
                print(f"[BARRIER ERROR] Neighbor finding failed for {vehicle_id}: {e}")
                return False
        
        # Execute neighbor finding in parallel
        if self.parallel_manager.num_workers > 1:
            with ThreadPoolExecutor(max_workers=self.parallel_manager.num_workers) as executor:
                futures = [executor.submit(find_neighbors_for_vehicle, vid) 
                          for vid in self.parallel_manager.vehicle_buffer.get_all_vehicle_ids()]
                success_results = [future.result() for future in as_completed(futures, timeout=15)]
        else:
            success_results = [find_neighbors_for_vehicle(vid) 
                             for vid in self.parallel_manager.vehicle_buffer.get_all_vehicle_ids()]
        
        # BARRIER 2: Wait for neighbor finding to complete
        if self.parallel_manager.processing_barrier:
            try:
                self.parallel_manager.processing_barrier.wait_all_complete("Neighbor Finding")
            except Exception as e:
                print(f"[BARRIER ERROR] Phase 2 barrier failed: {e}")
                return False
        
        # PHASE 3: Calculate performance metrics using consistent data
        print(f"[BARRIER] Phase 3: Calculating performance metrics")
        
        def calculate_performance_for_vehicle(vehicle_id):
            """Calculate performance using consistent buffer data"""
            try:
                input_data = self.parallel_manager.vehicle_buffer.get_input_data(vehicle_id)
                neighbors = self.parallel_manager.vehicle_buffer.get_temp_calculation(vehicle_id, 'neighbors')
                
                if not input_data or neighbors is None:
                    return False
                
                # Calculate CBR using consistent data
                cbr = self._calculate_cbr_with_buffer_data(vehicle_id, neighbors, input_data)
                
                # Calculate SINR using consistent data
                sinr = self.interference_calculator.calculate_sinr_with_interference(
                    vehicle_id, neighbors, input_data['transmission_power'], 
                    self.config.channel_model, 
                    getattr(self, 'background_traffic_manager', None)
                )
                
                # Calculate performance metrics
                metrics = self._calculate_performance_metrics_rl_aware(
                    vehicle_id, sinr, cbr, neighbors, self.config.channel_model
                )
                ...
                self.parallel_manager.vehicle_buffer.set_final_result(vehicle_id, final_results)
                return True
                
            except Exception as e:
                print(f"[BARRIER ERROR] Performance calculation failed for {vehicle_id}: {e}")
                return False
        
        # Execute performance calculations in parallel
        if self.parallel_manager.num_workers > 1:
            with ThreadPoolExecutor(max_workers=self.parallel_manager.num_workers) as executor:
                futures = [executor.submit(calculate_performance_for_vehicle, vid) 
                          for vid in self.parallel_manager.vehicle_buffer.get_all_vehicle_ids()]
                final_results = [future.result() for future in as_completed(futures, timeout=20)]
        else:
            final_results = [calculate_performance_for_vehicle(vid) 
                            for vid in self.parallel_manager.vehicle_buffer.get_all_vehicle_ids()]
        
        # BARRIER 3: Wait for performance calculations to complete  
        if self.parallel_manager.processing_barrier:
            try:
                self.parallel_manager.processing_barrier.wait_all_complete("Performance Calculation")
            except Exception as e:
                print(f"[BARRIER ERROR] Phase 3 barrier failed: {e}")
                return False
        
        # PHASE 4: Apply results to actual vehicle objects (serial for safety)
        print(f"[BARRIER] Phase 4: Applying results to vehicle objects")
        
        applied_count = 0
        for vehicle_id in self.parallel_manager.vehicle_buffer.get_all_vehicle_ids():
            try:
                if vehicle_id not in self.vehicles:
                    continue
                ...
                input_data = self.parallel_manager.vehicle_buffer.get_input_data(vehicle_id)
                results = self.parallel_manager.vehicle_buffer.get_final_result(vehicle_id)
                ...
                vehicle.x = input_data['x']
                vehicle.y = input_data['y'] 
                vehicle.speed = input_data['speed']
                vehicle.angle = input_data['angle']
                
                # Apply calculated results
                vehicle.neighbors = results['neighbors']
                vehicle.neighbors_number = len(results['neighbors'])
                vehicle.current_cbr = results['cbr']
                vehicle.current_snr = results['sinr']
                ...
                vehicle.current_latency = metrics.get('latency', 0)
                ...
                if self.sdn_controller:
                    ...
                    self.sdn_controller.update_network_topology(vehicle_id, vehicle.neighbors)
                
                applied_count += 1
                
            except Exception as e:
                print(f"[BARRIER ERROR] Result application failed for {vehicle_id}: {e}")
        
        print(f"[BARRIER] Successfully processed {applied_count}/{len(current_vehicles)} vehicles")
        return applied_count > 0
    
    def _find_neighbors_with_buffer_data(self, vehicle_id: str, current_time: float, input_data: dict) -> List[Dict]:
        """Find neighbors using consistent buffer data"""
        vehicle_pos = (input_data['x'], input_data['y'])
        vehicle_heading = input_data['angle']
        
        # Get ALL vehicle positions from buffer for consistency
        all_vehicle_ids = self.parallel_manager.vehicle_buffer.get_all_vehicle_ids()
        
        neighbors = []
        for other_vehicle_id in all_vehicle_ids:
            if other_vehicle_id == vehicle_id:
                continue
                
            other_data = self.parallel_manager.vehicle_buffer.get_input_data(other_vehicle_id)
            if not other_data:
                continue
            ...
            # Use existing neighbor finding logic but with buffer data
            # (Simplified version - use your existing _find_neighbors logic here)
            if distance <= 200:  # Basic range check - replace with your antenna logic
                neighbor_info = {
                    'id': other_vehicle_id,
                    'distance': distance,
                    'tx_power': other_data['transmission_power'],
                    'beacon_rate': other_data['beacon_rate'],
                    'mcs': other_data['mcs'],
                    'position': other_pos,
                    'link_quality': 'good'
                }
    ...
    def _calculate_cbr_with_buffer_data(self, vehicle_id: str, neighbors: List[Dict], input_data: dict) -> float:
        """Calculate CBR using consistent buffer data"""
        # Use existing CBR calculation logic but with buffer data
        # This ensures all vehicles use the SAME neighbor data snapshot
        
        num_neighbors = len(neighbors)
        vehicle_beacon_rate = input_data['beacon_rate']
        
        # Simplified CBR calculation - replace with your full logic
        base_cbr = 0.02
        
        if num_neighbors == 0:
            return base_cbr + (vehicle_beacon_rate * 0.001)
        
        # Calculate using consistent neighbor data
        total_beacon_load = vehicle_beacon_rate
        for neighbor in neighbors:
            total_beacon_load += neighbor['beacon_rate']
        
        cbr = base_cbr + (total_beacon_load * 0.001) + (num_neighbors * 0.01)
        
        # Apply background traffic (if manager exists)
        if hasattr(self, 'background_traffic_manager') and self.background_traffic_manager:
            cbr = self.background_traffic_manager.get_effective_cbr_contribution(vehicle_id, cbr)
        
        return min(1.0, cbr)  # Cap at 1.0
    ...
    def __init__(self, config: SimulationConfig, fcd_file: str, enable_rl: bool = False, 
        ...
        # Initialize parallel processing performance tracking
        ...
        def _initialize_parallel_processing_in_init(self):
            """Add this to the __init__ method of VANET_IEEE80211bd_L3_SDN_Simulator"""
    ...
    def create_vehicle_batch_from_current_vehicles(self, current_vehicles, current_time):
        ...
        for vehicle_id in current_vehicles:
            ...
            # Get vehicle data from FCD
            vehicle_data = next((data for data in self.mobility_data 
                               if data['time'] == current_time and data['id'] == vehicle_id), None)
            
            if vehicle_data:
                vehicle_ids.append(vehicle_id)
                positions.append([vehicle_data['x'], vehicle_data['y']])
                headings.append(vehicle_data.get('angle', 0))
                powers.append(getattr(vehicle, 'transmission_power', 20.0))
                mcs_values.append(getattr(vehicle, 'mcs', 1))
                beacon_rates.append(getattr(vehicle, 'beacon_rate', 10.0))
        ...
        return VehicleDataBatch(
            ...
            beacon_rates=np.array(beacon_rates)
        )
    ...
    def _process_vehicles_parallel_or_serial(self, current_vehicles, current_time):
        ...
        if processing_mode == 'serial' or not ENABLE_PARALLEL_PROCESSING:
            # Serial processing (existing code)
            self._process_vehicles_serial_fallback(current_vehicles, current_time)
        else:
            # NEW: Barrier-synchronized parallel processing
            try:
                success = self._process_vehicles_with_barriers(current_vehicles, current_time)
                if not success:
                    print("[PARALLEL] Barrier processing failed, falling back to serial")
                    self.parallel_manager.adaptive_fallback("Barrier processing")
                    self._process_vehicles_serial_fallback(current_vehicles, current_time)
            except Exception as e:
                print(f"[PARALLEL ERROR] Barrier processing failed: {e}")
                self.parallel_manager.adaptive_fallback("Barrier processing exception")
                self._process_vehicles_serial_fallback(current_vehicles, current_time)
        
        end_time = time.time()
        self.monitor_parallel_performance(start_time, end_time, len(current_vehicles), processing_mode)

    def _process_vehicles_serial_fallback(self, current_vehicles, current_time):
        """Serial processing fallback using existing methods"""
        for vehicle_id in current_vehicles:
            if vehicle_id not in self.vehicles:
                continue
            ...
            # Find neighbors using existing method
    ...
    def _calculate_performance_parallel_or_serial(self, current_vehicles):
        """Enhanced performance calculation with parallel capability"""
        start_time = time.time()  # ADD THIS LINE
        
        # Determine processing mode
        computation_mode = 'serial'
        if (hasattr(self, 'parallel_manager') and self.parallel_manager and 
            len(current_vehicles) >= MIN_VEHICLES_FOR_PARALLEL and
            ENABLE_PARALLEL_PROCESSING):
            computation_mode = self.parallel_manager.get_computation_mode()
        
        # For now, use serial calculation (can be enhanced later)
        for vehicle_id in current_vehicles:
            if vehicle_id not in self.vehicles:
                continue
            ...
            # Calculate CBR and SINR using existing methods
            vehicle.current_cbr = self._calculate_cbr_realistic_dynamic(vehicle_id, vehicle.neighbors)
            
            # Use correct transmission power for SINR calculation
            ...
            # Calculate performance metrics using existing method
            metrics = self._calculate_performance_metrics_rl_aware(
                vehicle_id, vehicle.current_snr, vehicle.current_cbr, 
                vehicle.neighbors, self.config.channel_model
            )
        ...
        # ADD THIS CALL
        self.monitor_parallel_performance(start_time, end_time, len(current_vehicles), computation_mode)
    ...
    def run_simulation(self) -> List[Dict]:
        """Run enhanced simulation with parallel processing capabilities"""
        print(f"[INFO] Starting Enhanced IEEE 802.11bd VANET simulation with parallel processing")
        print(f"[INFO] Parallel mode: {self.parallel_manager.get_computation_mode() if self.parallel_manager else 'Disabled'}")
        ...
        for time_idx, current_time in enumerate(time_points):
            ...
            if self.fcd_reload_count > 1:
                ...
                if time_idx % 50 == 0:
                    progress = time_idx / len(time_points) * 100
                    mode_info = f" | Mode: {self.parallel_manager.get_computation_mode()}" if self.parallel_manager else ""
                    print(f"[PROGRESS] Processing t={current_time:.1f}s ({time_idx+1}/{len(time_points)}) - {progress:.1f}%{mode_info}")
            
            # Phase 0: Apply RL updates from previous timestamp
            if pending_rl_updates:
                self._apply_pending_rl_updates(pending_rl_updates)
                
                # Ensure antenna systems are synchronized with transmission_power
                for vehicle in self.vehicles.values():
            ...
            # Store RL parameters before processing
            rl_preserved_params = {}
            if self.enable_rl:
                for vehicle_id in current_vehicles:
            ...
            # Phase 1: Update attacker behavior
            if self.attack_manager:
            ...
            # Phase 2: Update positions and find neighbors (PARALLEL OPTIMIZED)
            self._process_vehicles_parallel_or_serial(current_vehicles, current_time)

            
            # Phase 2.5: Generate background traffic
            ...
            # Phase 3: Enhanced Layer 3 Protocol Updates
            if self.config.enable_layer3:
                self._update_all_routing_protocols(current_vehicles, current_time)
            
            # Phase 4: Packet Generation and Processing
            ...
            # Phase 5: SDN Traffic Engineering
            ...
            # Phase 6: RL Optimization (store response for next timestamp)
            if self.enable_rl and self.rl_client:
                ...
                for vehicle_id in current_vehicles:
                    ...
                    rl_vehicle_data = {
                        ...
                        'processing_speedup': parallel_performance_stats.get('parallel_speedup', 1.0)
                    }
                ...
                try:
                    ...
                    if rl_response and 'vehicles' in rl_response:
                        pending_rl_updates = rl_response
                        
                        if time_idx % 100 == 0:
                            updated_vehicles = len(rl_response['vehicles'])
                            antenna_info = f"({ANTENNA_TYPE})"
                            parallel_info = f" | Parallel: {self.parallel_manager.get_computation_mode()}" if self.parallel_manager else ""
                            print(f"[RL] Received response for {updated_vehicles} vehicles {antenna_info}{parallel_info}")
                
                except Exception as e:
                    if time_idx % 100 == 0:
                        print(f"[RL ERROR] Communication failed at t={current_time:.2f}: {e}")
            
            # Phase 7: Calculate CBR and SINR (PARALLEL OPTIMIZED)
            self._calculate_performance_parallel_or_serial(current_vehicles)
            
            # Phase 8: Update SDN metrics
            if self.config.enable_sdn:
                for vehicle_id in current_vehicles:
                    if vehicle_id in self.vehicles:
                        self._update_sdn_metrics(vehicle_id, current_time)
            
            # Phase 9: Calculate performance metrics and create results
            timestamp_results = []
            ...
            for vehicle_id in current_vehicles:
                ...
                if random.random() < 0.002:
                    ...
                    parallel_info = f"Parallel:{self.parallel_manager.get_computation_mode()}" if self.parallel_manager else "Serial"
                    
                    print(f"[ENHANCED DEBUG] {episode_info}Vehicle {vehicle_id} | {l3_info} | {sdn_info} | {parallel_info}")
                    print(f"  Neighbors: {len(vehicle.neighbors)} | SINR: {vehicle.current_snr:.1f} dB | CBR: {vehicle.current_cbr:.3f}")
                    print(f"  PER: {vehicle.current_per:.4f} | PDR: {vehicle.current_pdr:.4f} | Throughput: {vehicle.current_throughput/1e6:.2f} Mbps")
                    
                    if self.enable_rl:
                        print(f"  RL Params: Power={vehicle.transmission_power:.1f}dBm, MCS={vehicle.mcs}, Beacon={vehicle.beacon_rate:.1f}Hz")
                
                # Create enhanced result record (use existing result creation logic)
                result = self._create_enhanced_result_record(vehicle_id, vehicle, current_time, current_episode)
                
                timestamp_results.append(result)
                results.append(result)
            
            # Monitor GPU memory if using GPU
            if time_idx % 50 == 0 and self.parallel_manager:
                self._monitor_gpu_memory()
            
            # Phase 10: Update visualization
            ...
            if time_idx % 100 == 0 and timestamp_results:
                ...
                parallel_info = f"Mode: {self.parallel_manager.get_computation_mode()} | " if self.parallel_manager else ""
                print(f"[ENHANCED VALIDATION] {episode_info}{parallel_info}t={current_time:.1f}s:")
        ...
        if hasattr(self, 'parallel_performance_stats') and self.parallel_performance_stats:
            ...
            print(f"  Modes used: {', '.join(perf_summary['modes_used'])}")
            print(f"  Total processing time: {perf_summary['total_processing_time']:.2f}s")
            
            for mode, throughput in perf_summary['average_throughput'].items():
                peak = perf_summary['peak_throughput'].get(mode, 0)
                print(f"  {mode.upper()}: Avg={throughput:.1f} veh/s, Peak={peak:.1f} veh/s")
            
            if perf_summary['mode_efficiency_ranking']:
                best_mode, best_efficiency = perf_summary['mode_efficiency_ranking'][0]
                print(f"  Best performing mode: {best_mode.upper()} ({best_efficiency:.1f} veh/ms)")
            
            print(f"  Recommendation: {perf_summary['recommendation']}")
            
            if perf_summary.get('fallback_count', 0) > 0:
                print(f"  Fallback events: {perf_summary['fallback_count']}")
        ...
        if self.parallel_manager:
            total_parallel_time = (parallel_performance_stats['gpu_time'] + 
                                  parallel_performance_stats['cpu_time'])
            total_serial_time = parallel_performance_stats['serial_time']
            
            if total_serial_time > 0 and total_parallel_time > 0:
                parallel_performance_stats['parallel_speedup'] = total_serial_time / total_parallel_time
            
            print(f"\n[PARALLEL PERFORMANCE SUMMARY]")
            print(f"  GPU Time: {parallel_performance_stats['gpu_time']:.2f}s")
            print(f"  CPU Parallel Time: {parallel_performance_stats['cpu_time']:.2f}s") 
            print(f"  Serial Time: {parallel_performance_stats['serial_time']:.2f}s")
            print(f"  Fallback Count: {parallel_performance_stats['fallback_count']}")
            print(f"  Overall Speedup: {parallel_performance_stats['parallel_speedup']:.2f}x")
        
        self.simulation_results = results
        print(f"[INFO] Enhanced IEEE 802.11bd simulation with parallel processing completed.")
        print(f"[INFO] Generated {len(results)} data points in {len(time_points)} time steps.")
    ...
    def _safe_call_routing_method(self, protocol, method_name: str, *args, **kwargs):
        ...
        try:
            ...
            return None
    def _process_vehicles_serial(self, current_vehicles: Set[str], current_time: float):
        """Process vehicles using serial neighbor finding"""
        for vehicle_id in current_vehicles:
            if vehicle_id not in self.vehicles:
                continue
    ...
    def _calculate_performance_serial(self, current_vehicles: Set[str]):
        """Calculate performance metrics using serial processing"""
        for vehicle_id in current_vehicles:
            if vehicle_id not in self.vehicles:
                continue
    ...
    def _monitor_gpu_memory(self):
        ...
        if GPU_AVAILABLE and self.parallel_manager and self.parallel_manager.get_computation_mode() == 'gpu':
            try:
                ...
                if used_bytes > 0 and total_bytes > 0:
                    usage_percent = (used_bytes / total_bytes) * 100
                    
                    # Progressive cleanup strategy
                    if usage_percent > 90:
                        print(f"[GPU MEMORY CRITICAL] {usage_percent:.1f}% - aggressive cleanup")
                        import gc
                        gc.collect()
                        ...
                        print(f"[GPU MEMORY INFO] {usage_percent:.1f}% - monitoring")
                    
                    # Track memory usage in performance stats
                    if hasattr(self, 'parallel_performance_stats'):
                        if 'gpu_memory_history' not in self.parallel_performance_stats:
                            self.parallel_performance_stats['gpu_memory_history'] = []
                        
                        self.parallel_performance_stats['gpu_memory_history'].append({
                            'timestamp': time.time(),
    ...
    def monitor_parallel_performance(self, start_time, end_time, vehicle_count, mode):
        ...
        # Adaptive logging - more verbose for significant processing or performance issues
        should_log = (
            vehicle_count > 20 or  # Significant vehicle count
            processing_time > 0.1 or  # Slow processing
            vehicles_per_second < 100 or  # Low throughput
            len(self.parallel_performance_stats[mode]) % 50 == 1  # Periodic logging
        )
        
        if should_log:
            efficiency = perf_record['efficiency_score']
            batch_util = perf_record['batch_utilization']
            
            # Performance classification
            if vehicles_per_second > 1000:
                perf_class = "EXCELLENT"
            elif vehicles_per_second > 500:
                perf_class = "GOOD"
            elif vehicles_per_second > 200:
                perf_class = "FAIR"
            else:
                perf_class = "POOR"
            
            print(f"[PARALLEL PERF] {mode.upper()}: {processing_time:.4f}s for {vehicle_count} vehicles "
                  f"({vehicles_per_second:.1f} veh/s, {efficiency:.1f} veh/ms) - {perf_class}")
            
            # Additional diagnostics for poor performance
            if perf_class in ["FAIR", "POOR"] and mode == "gpu":
                print(f"[PARALLEL DIAG] GPU batch utilization: {batch_util:.2f}, consider batch size tuning")
        
        # Keep only recent performance data (sliding window)
        max_records = 200
        if len(self.parallel_performance_stats[mode]) > max_records:
            self.parallel_performance_stats[mode] = self.parallel_performance_stats[mode][-max_records:]
    ...
    def get_parallel_performance_summary(self):
        ...
        # Rank modes by efficiency
        if mode_performance:
            summary['mode_efficiency_ranking'] = sorted(mode_performance.items(), 
